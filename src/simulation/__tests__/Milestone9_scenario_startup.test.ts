...
describe('Milestone 9 clean scenario startup', () => {
  test('all-ON startup creates the default 27/26/26 CARTBUILD allocation exactly once', () => {
    ...
    expect(state.missions.every((mission) => mission.missionType === 'CARTBUILD' && mission.createdAtSec === 0)).toBe(true)
    expect(state.srsControl).toMatchObject({ planningCadenceSec: 10, nextPlanningTime: 10 })
  })

  test('Körber-only startup creates 27/26/26 EMPTY missions and no CARTBUILD missions', () => {
    const engine = new SimulationEngine(SEGMENTS)
    engine.startScenario({ ...ALL_ON, cartbuildAEnabled: false, cartbuildBEnabled: false, cartbuildCEnabled: false }, 10)
    expect((['A', 'B', 'C'] as SourceId[]).map((source) => missionCounts(engine, source, 'EMPTY'))).toEqual([27, 26, 26])
  ...
  test('mixed startup types A as CARTBUILD and B/C as EMPTY', () => {
    ...
    expect([missionCounts(engine, 'A', 'CARTBUILD'), missionCounts(engine, 'B', 'EMPTY'), missionCounts(engine, 'C', 'EMPTY')]).toEqual([27, 26, 26])
    expect(missionCounts(engine, 'A', 'EMPTY')).toBe(0)
  ...
  test('all toggles OFF creates no missions and remains balanced', () => {
    ...
    expect(state.srsControl).toMatchObject({ globalPending: 0, globalAvailableCapacity: 79, nextPlanningTime: 10 })
  })
  ...
  test('selected cadence is installed before planning and normal Reset restores defaults', () => {
    ...
    expect([reset.pendingA, reset.pendingB, reset.pendingC]).toEqual([27, 26, 26])
    expect(reset.missions.every((mission) => mission.missionType === 'CARTBUILD')).toBe(true)
