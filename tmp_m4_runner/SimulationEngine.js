...
export class SimulationEngine {
    ...
    // merge arbitration
    mergeState;
    constructor(segmentConfigs, options) {
    ...
    processTick(deltaSec) {
        ...
        for (const t of traysOrder) {
            if (t.status !== 'MOVING' && t.status !== 'BLOCKED')
                continue;
    ...
    findDownstreamNeighborOnSameSegment(tray) {
    ...
    transferFromFeederToT(sourceId) {
        ...
        if (sourceId === 'A' || sourceId === 'B' || sourceId === 'C') {
            ...
            if (sourceId === 'A')
                this.mergeState.cumulativeTransfersA += 1;
            if (sourceId === 'B')
                this.mergeState.cumulativeTransfersB += 1;
            if (sourceId === 'C')
                this.mergeState.cumulativeTransfersC += 1;
        }
    }
    advanceMergePointer(sourceId) {
        if (sourceId === 'A')
            this.mergeState.nextPriority = 'B';
        else if (sourceId === 'B')
            this.mergeState.nextPriority = 'C';
        else
            this.mergeState.nextPriority = 'A';
    }
    tryTransferToNextSegment(t, currentSegmentId, nextId, distance) {
    ...
    allowedEntryDistance(segmentId) {
        ...
        if (seg.maxOccupancy !== undefined && occupancy >= seg.maxOccupancy)
            return 0;
        if (traysOn.length === 0)
            return Infinity;
    ...
    canEnterSegment(segmentId, desiredEntryDist) {
        const seg = this.segmentsMap.get(segmentId).config;
        const traysOn = this.trays.filter((t) => t.currentSegmentId === segmentId);
        const occupancy = traysOn.length;
        if (seg.maxOccupancy !== undefined && occupancy >= seg.maxOccupancy)
